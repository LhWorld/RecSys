...
from recommmend import UserCF,ItemCF,ItemCF_IUF,Evaluation,LFM
from db import data_helpers
import random
...
def SplitData(data,numFlod,k,seed):
    #data,numFlod 5,i k和i相同 0,1,2,3,4
    test = []
    train = []
    random.seed(seed)
    for user, item,rating in data:
        if random.randint(0,numFlod-1) == k:
...
if __name__ == '__main__':
    ...
    coverage = 0
    popularity =0
    # for i in range(1,2):#0,numFlod
    # [oriTrain,oriTest] = SplitData(data,numFlod,i,0)
    oriTrain, oriTest = train_test_split(data, test_size=0.3)
...
#        result =  ItemCF_IUF.Recommendation(test.keys(),train, W)
    ...
    [P,Q] = LFM.LatentFactorModel(train, 10,30, 0.02, 0.01)#分10类 30是
